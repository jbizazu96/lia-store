...
function requireComplete(data: FirebaseFirestore.DocumentData) {
  ...
  if (!text(data.firstName) || !text(data.lastName) || !/^\(\d{3}\) \d{3} - \d{4}$/.test(text(data.phone)) || !text(data.email) || age(text(data.dateOfBirth)) < MINIMUM_AGE) missing.push("personal information");
  ...
  if (!text(area.city) || !state(area.state) || typeof area.preferredRadiusMiles !== "number" || area.preferredRadiusMiles <= 0 || area.preferredRadiusMiles > MAXIMUM_RADIUS) missing.push("service area");
  ...
  if (!text(license.frontDocumentUrl) || !text(license.backDocumentUrl) || !state(license.issuingState) || !futureDate(text(license.expirationDate))) missing.push("driver's license");
  if (!text(insurance.documentUrl) || !futureDate(text(insurance.expirationDate))) missing.push("vehicle insurance");
  if (!text(registration.documentUrl) || !futureDate(text(registration.expirationDate))) missing.push("vehicle registration");
  ...
  if (!text(data.stripeAccountId)) missing.push("Stripe payout setup");
...
export const getDriverOnboardingDraft = onCall({ region: "us-central1" }, async (request) => {
  ...
  return safeDraft(request.auth.uid, driver.data());
});
...
export const saveDriverPersonalInformation = onCall({ region: "us-central1" }, async (request) => {
  ...
  if (!firstName || !lastName || !/^\(\d{3}\) \d{3} - \d{4}$/.test(phone) || !/^\S+@\S+\.\S+$/.test(email) || !dateOfBirth || age(dateOfBirth) < MINIMUM_AGE) throw new HttpsError("invalid-argument", "Enter complete personal information. Drivers must be at least 18 years old.");
...
export const saveDriverAddressAndServiceArea = onCall({ region: "us-central1", secrets: [googleMapsApiKey] }, async (request) => {
  ...
  if (!text(address.street) || !text(address.city) || !addressState || !text(address.zip) || !text(area.city) || !areaState || !radius || radius <= 0 || radius > MAXIMUM_RADIUS) throw new HttpsError("invalid-argument", `Select a preferred service radius between 1 and ${MAXIMUM_RADIUS} miles.`);
...
export const saveDriverDocuments = onCall({ region: "us-central1" }, async (request) => {
  ...
  if (!issuingState || !present("drivers-license-front", license.frontDocumentUrl) || !present("drivers-license-back", license.backDocumentUrl) || !present("vehicle-insurance", insurance.documentUrl) || !present("vehicle-registration", registration.documentUrl) || !futureDate(licenseExpirationDate) || !futureDate(insuranceExpirationDate) || !futureDate(registrationExpirationDate)) throw new HttpsError("invalid-argument", "Provide every required document and future expiration date.");
...
export const completeDriverOnboarding = onCall({ region: "us-central1" }, async (request) => {
  if (!request.auth) throw new HttpsError("unauthenticated", "Sign in to submit onboarding."); const { driver } = await driverFor(request.auth.uid); const data = driver.data() ?? {}; requireComplete(data);
