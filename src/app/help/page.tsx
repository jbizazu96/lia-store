...
export default function HelpPage() {
  ...
  const handleBack = () => {
    ...
    try {
      ...
      /* A malformed referrer uses the safe app fallback below. */
    }
  ...
  return (
    <main className="min-h-screen bg-slate-50 px-4 py-10">
      <section className="mx-auto max-w-xl rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-100">
        <button type="button" onClick={handleBack} className="inline-flex items-center gap-2 text-sm font-semibold text-orange-600"><ArrowLeft className="h-4 w-4" />Back to LIA</button>
