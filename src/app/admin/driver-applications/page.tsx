...
export default async function AdminDriverApplicationsPage({searchParams}: {searchParams: Promise<{status?: string}>}) {
  const requested = (await searchParams).status as AdminApplicationStatus | undefined;
  return <ApplicationReviewWorkspace type="driver" initialStatus={requested && statuses.has(requested) ? requested : "pending_review"} />;
}
