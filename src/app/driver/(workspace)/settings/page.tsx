...
export default function DriverSettingsPage() {
  ...
  return <section className="mx-auto max-w-3xl"><p className="text-sm font-semibold text-orange-600">DRIVER SETTINGS</p><h1 className="mt-1 text-3xl font-bold">Settings</h1>{message && <p className="mt-4 rounded-xl bg-green-50 p-3 text-sm text-green-800">{message}</p>}<input ref={profileImageInput} className="hidden" type="file" accept="image/jpeg,image/png,image/webp,image/avif" onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadProfilePhoto(file); }} />
    ...
    <article className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-100"><p className="font-bold">Support</p><div className="mt-3 grid gap-2"><a href="/help" className="flex items-center justify-between rounded-xl px-2 py-2 text-sm font-medium hover:bg-slate-50">Help Center <HelpCircle className="h-4 w-4" /></a><a href="mailto:support@liamarketplace.com" className="flex items-center justify-between rounded-xl px-2 py-2 text-sm font-medium hover:bg-slate-50">Contact Support <Mail className="h-4 w-4" /></a><a href="mailto:support@liamarketplace.com?subject=Driver%20issue" className="flex items-center justify-between rounded-xl px-2 py-2 text-sm font-medium hover:bg-slate-50">Report an issue <AlertCircle className="h-4 w-4" /></a></div></article>
