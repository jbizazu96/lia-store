...
function DriverOnboardingStepContent({
  ...
  const canFinishStripe = stripeStatus === "complete" || stripeStatus === "pending_verification";
  const canSave = step === "personal-information"
    ...
    : step === "address-service-area"
      ? Boolean(address.street.trim() && address.city.trim() && address.state.trim() && address.zip.trim() && serviceArea.city.trim() && serviceArea.state.trim() && Number(serviceArea.preferredRadiusMiles) > 0)
      : step === "vehicle-information"
        ...
        : step === "documents"
          ? Boolean(documents.issuingState.trim() && documents.licenseExpirationDate && documents.insuranceExpirationDate && documents.registrationExpirationDate && (files["drivers-license-front"] || draft.driversLicense.frontDocumentUrl) && (files["vehicle-insurance"] || draft.vehicleInsurance.documentUrl) && (files["vehicle-registration"] || draft.vehicleRegistration.documentUrl))
          : step === "agreement"
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-green-50 px-4 py-8">
      <section className="mx-auto max-w-2xl rounded-3xl bg-white p-6 shadow-xl sm:p-8">
        ...
        {step === "documents" ? <div className="space-y-6"><div className="rounded-2xl bg-amber-50 p-4 text-sm text-amber-900"><FileText className="mr-2 inline h-4 w-4" />Use current, readable documents. Your documents are used only for LIA's private approval review.</div><div className="grid gap-4 sm:grid-cols-2"><Input label="Driver license issuing state" required value={documents.issuingState} onChange={(event) => setDocuments({ ...documents, issuingState: event.target.value })} /><Input label="License expiration date" required type="date" value={documents.licenseExpirationDate} onChange={(event) => setDocuments({ ...documents, licenseExpirationDate: event.target.value })} /><DocumentUpload label="Driver license front" required preview={previews["drivers-license-front"] || draft.driversLicense.frontDocumentUrl} onSelect={(file) => setFile("drivers-license-front", file)} /><DocumentUpload label="Driver license back (optional)" preview={previews["drivers-license-back"] || draft.driversLicense.backDocumentUrl} onSelect={(file) => setFile("drivers-license-back", file)} /></div><div className="grid gap-4 border-t border-gray-100 pt-6 sm:grid-cols-2"><Input label="Insurance provider (optional)" value={documents.insuranceProvider} onChange={(event) => setDocuments({ ...documents, insuranceProvider: event.target.value })} /><Input label="Insurance expiration date" required type="date" value={documents.insuranceExpirationDate} onChange={(event) => setDocuments({ ...documents, insuranceExpirationDate: event.target.value })} /><DocumentUpload label="Vehicle insurance" required preview={previews["vehicle-insurance"] || draft.vehicleInsurance.documentUrl} onSelect={(file) => setFile("vehicle-insurance", file)} /><div className="hidden sm:block" /></div><div className="grid gap-4 border-t border-gray-100 pt-6 sm:grid-cols-2"><Input label="Registration expiration date" required type="date" value={documents.registrationExpirationDate} onChange={(event) => setDocuments({ ...documents, registrationExpirationDate: event.target.value })} /><DocumentUpload label="Vehicle registration" required preview={previews["vehicle-registration"] || draft.vehicleRegistration.documentUrl} onSelect={(file) => setFile("vehicle-registration", file)} /></div></div> : null}
        ...
        {step === "stripe" ? <div className="space-y-5"><div className="rounded-2xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-900">Stripe securely collects identity, tax, and bank information for payouts. LIA never receives your bank account details. After you submit Stripe's form, it may take time for Stripe to verify your information.</div><div className="rounded-xl bg-gray-50 p-4"><p className="font-semibold text-gray-800">Stripe status: <span className="capitalize">{(stripeStatus ?? "not_started").replaceAll("_", " ")}</span></p><p className="mt-1 text-sm text-gray-500">{canFinishStripe ? "Your Stripe submission has been received. Submit your driver application for LIA approval." : "Complete Stripe's hosted payout form to continue."}</p></div>{canFinishStripe ? <button type="button" onClick={async () => { setSaving(true); try { await driverOnboardingService.complete(user.uid); router.replace("/driver/pending-approval"); } catch (cause) { setMessage(cause instanceof Error ? cause.message : "Unable to submit your application."); } finally { setSaving(false); } }} disabled={saving} className="w-full rounded-xl bg-green-600 py-3 font-semibold text-white disabled:opacity-50"><CheckCircle2 className="mr-2 inline h-5 w-5" />Submit application for approval</button> : <button type="button" onClick={connectStripe} disabled={connecting} className="w-full rounded-xl bg-indigo-600 py-3 font-semibold text-white disabled:opacity-50">{connecting ? "Opening Stripe..." : stripeStatus && stripeStatus !== "not_started" ? "Continue Stripe setup" : "Connect Stripe"}</button>}</div> : null}
