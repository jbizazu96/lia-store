...
function StateSelect({label, value, onChange}: {label: string; value: string; onChange: (value: string) => void}) {
  return <label className="block text-sm font-medium text-gray-700">{label} *<UsStateSelect required value={value} onChange={onChange} className="mt-1 w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 outline-none transition focus:border-orange-500 focus:ring-2 focus:ring-orange-100" /></label>;
}
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-green-50 px-4 py-8">
      <section className="mx-auto max-w-2xl rounded-3xl bg-white p-6 shadow-xl sm:p-8">
        ...
        {step === "address-service-area" ? <div className="grid gap-4 sm:grid-cols-2"><div className="sm:col-span-2"><Input label="Home address" required value={address.street} onChange={(event) => setAddress({ ...address, street: event.target.value })} /></div><Input label="Apartment, suite, etc. (optional)" value={address.apartment} onChange={(event) => setAddress({ ...address, apartment: event.target.value })} /><div className="hidden sm:block" /><Input label="City" required value={address.city} onChange={(event) => setAddress({ ...address, city: event.target.value })} /><StateSelect label="State" value={address.state} onChange={(state) => setAddress({ ...address, state })} /><Input label="ZIP code" required value={address.zip} onChange={(event) => setAddress({ ...address, zip: event.target.value })} /><div className="sm:col-span-2 mt-2 rounded-2xl bg-blue-50 p-4"><p className="font-semibold text-blue-900">Area you prefer to serve</p><div className="mt-3 grid gap-4 sm:grid-cols-3"><Input label="Service city" required value={serviceArea.city} onChange={(event) => setServiceArea({ ...serviceArea, city: event.target.value })} /><StateSelect label="Service state" value={serviceArea.state} onChange={(state) => setServiceArea({ ...serviceArea, state })} /><Input label="Requested radius (miles)" required type="number" min="1" max={maximumPreferredRadiusMiles} value={serviceArea.preferredRadiusMiles} onChange={(event) => setServiceArea({ ...serviceArea, preferredRadiusMiles: event.target.value })} /></div><p className="mt-3 text-sm text-blue-800">Drivers may request up to {maximumPreferredRadiusMiles} miles. An administrator approves the final operating radius.</p></div></div> : null}
