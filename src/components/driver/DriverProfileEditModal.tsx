...
export function DriverProfileEditModal({ profile, saving, onClose, onSave }: DriverProfileEditModalProps) {
  ...
  return <div className="fixed inset-0 z-50 flex items-end bg-slate-950/45 sm:items-center sm:justify-center sm:p-6" role="dialog" aria-modal="true" aria-label="Edit driver profile"><div className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-t-3xl bg-white p-5 shadow-2xl sm:rounded-3xl sm:p-6"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold tracking-wider text-orange-600">DRIVER PROFILE</p><h2 className="mt-1 text-2xl font-bold text-slate-950">Edit your information</h2><p className="mt-1 text-sm text-slate-500">Your email is used for sign-in and cannot be changed here.</p></div><button type="button" onClick={onClose} disabled={saving} className="rounded-full p-2 text-slate-500 hover:bg-slate-100" aria-label="Close"><X className="h-5 w-5" /></button></div>
    ...
    <section className="border-t border-slate-100 pt-6"><h3 className="font-bold text-slate-900">Home address</h3><p className="mt-1 text-sm text-slate-500">Changing this address verifies new coordinates for delivery eligibility.</p><div className="mt-3 grid gap-3 sm:grid-cols-2"><div className="sm:col-span-2"><Field label="Street address" required><input className={inputClass} value={form.address.street} onChange={(event) => update("address", { ...form.address, street: event.target.value })} /></Field></div><Field label="Apartment or unit"><input className={inputClass} value={form.address.apartment} onChange={(event) => update("address", { ...form.address, apartment: event.target.value })} /></Field><Field label="City" required><input className={inputClass} value={form.address.city} onChange={(event) => update("address", { ...form.address, city: event.target.value })} /></Field><Field label="State" required><input className={inputClass} maxLength={2} value={form.address.state} onChange={(event) => update("address", { ...form.address, state: event.target.value.toUpperCase() })} placeholder="IA" /></Field><Field label="ZIP code" required><input className={inputClass} inputMode="numeric" value={form.address.zip} onChange={(event) => update("address", { ...form.address, zip: event.target.value })} /></Field></div></section>
    <section className="border-t border-slate-100 pt-6"><h3 className="font-bold text-slate-900">Service area</h3><div className="mt-3 grid gap-3 sm:grid-cols-3"><Field label="City" required><input className={inputClass} value={form.serviceArea.city} onChange={(event) => update("serviceArea", { ...form.serviceArea, city: event.target.value })} /></Field><Field label="State" required><input className={inputClass} maxLength={2} value={form.serviceArea.state} onChange={(event) => update("serviceArea", { ...form.serviceArea, state: event.target.value.toUpperCase() })} placeholder="IA" /></Field><Field label="Preferred radius (miles)" required><input className={inputClass} type="number" min="1" max="50" value={form.serviceArea.preferredRadiusMiles ?? ""} onChange={(event) => update("serviceArea", { ...form.serviceArea, preferredRadiusMiles: event.target.value ? Number(event.target.value) : null })} /></Field></div></section>
