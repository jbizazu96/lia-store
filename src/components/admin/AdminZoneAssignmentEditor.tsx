...
export function AdminZoneAssignmentEditor({
  ...
  return <section className="mt-5 rounded-xl border border-blue-100 bg-blue-50/60 p-4">
    ...
    {accountType === "customer" && <div className="mt-4"><p className="text-sm font-bold text-slate-700">Approved order zones</p><p className="mt-1 text-xs text-slate-500">Allows this customer to order from stores in these zones even when the trusted route exceeds the normal delivery radius. Pricing comes from the order zone and uses the full route miles.</p><div className="mt-2 grid gap-2 sm:grid-cols-2">{zones.filter((zone) => zone.id !== home).map((zone) => <label key={zone.id} className="flex items-center gap-2 rounded-lg border border-blue-100 bg-white px-3 py-2 text-sm font-semibold"><input type="checkbox" checked={orders.includes(zone.id)} onChange={() => toggleOrder(zone.id)} disabled={disabled || saving}/>{zone.name}</label>)}</div></div>}
