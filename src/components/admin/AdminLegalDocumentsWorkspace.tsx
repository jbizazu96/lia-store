...
export function AdminLegalDocumentsWorkspace() {
  ...
  const deleteDraft = async (document: ManagedLegalDocument) => {
    if (!await confirm({
      title: "Delete legal-document draft?",
      description: `Delete ${document.title} ${document.version}? This draft cannot be recovered.`,
      confirmationLabel: "Delete draft",
      tone: "danger",
    })) return;
    await mutate(() => adminLegalDocumentsClientService.deleteDraft(document.id));
  };
  const archiveDocument = async (document: ManagedLegalDocument) => {
    if (!await confirm({
      title: "Archive legal-document version?",
      description: `${document.title} ${document.version} will no longer be the active public version. Existing acceptance evidence will be retained.`,
      confirmationLabel: "Archive version",
      tone: "warning",
    })) return;
    await mutate(() => adminLegalDocumentsClientService.archive(document.id));
  };
  const importCurrentDocuments = () => void mutate(async () => {
    const has = (key: string, version: string) => documents.some((document) => document.documentKey === key && document.version === version);
    if (!has("customer_terms", CUSTOMER_TERMS_VERSION)) await adminLegalDocumentsClientService.create({documentKey: "customer_terms", title: "Customer Terms of Service", audience: "customer", version: CUSTOMER_TERMS_VERSION, content: plainText(customerTermsIntroduction, customerTermsSections), effectiveDate: CUSTOMER_TERMS_EFFECTIVE_DATE, lastUpdated: CUSTOMER_TERMS_LAST_UPDATED, changeSummary: "Adds pickup eligibility and verification, fulfillment-selection rules, pickup-specific customer responsibilities, and Stripe Tax estimate and final-calculation disclosures.", requiresAcceptance: true});
    if (!has("customer_privacy", CUSTOMER_PRIVACY_VERSION)) await adminLegalDocumentsClientService.create({documentKey: "customer_privacy", title: "Privacy Policy", audience: "customer", version: CUSTOMER_PRIVACY_VERSION, content: plainText(customerPrivacyIntroduction, customerPrivacySections), effectiveDate: CUSTOMER_PRIVACY_EFFECTIVE_DATE, lastUpdated: CUSTOMER_PRIVACY_LAST_UPDATED, changeSummary: "Adds pickup and Stripe Tax data practices, clarifies address-derived location, and discloses Apple and Google sign-in, App Check, reCAPTCHA Enterprise, Crashlytics, notification visibility, and provider responsibilities.", requiresAcceptance: true});
    if (!has("customer_refund_policy", CUSTOMER_REFUND_VERSION)) await adminLegalDocumentsClientService.create({documentKey: "customer_refund_policy", title: "Refund and Cancellation Policy", audience: "customer", version: CUSTOMER_REFUND_VERSION, content: plainText(customerRefundIntroduction, customerRefundSections), effectiveDate: CUSTOMER_REFUND_EFFECTIVE_DATE, lastUpdated: CUSTOMER_REFUND_LAST_UPDATED, changeSummary: "Initial managed version aligned with the enforced cancellation and claim workflow", requiresAcceptance: false});
  });

  const baselineMissing = ![
    ["customer_terms", CUSTOMER_TERMS_VERSION],
    ["customer_privacy", CUSTOMER_PRIVACY_VERSION],
    ["customer_refund_policy", CUSTOMER_REFUND_VERSION],
  ].every(([key, version]) => documents.some((document) => document.documentKey === key && document.version === version));

  return <div className="space-y-6 p-4 sm:p-6 lg:p-8">
    <header className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-widest text-orange-600">Governance</p><h1 className="mt-1 text-2xl font-black">Legal documents</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Manage customer policies and agreement templates by version. Published versions are immutable and retained for acceptance evidence.</p></div>{writable ? <div className="flex flex-wrap gap-2">{baselineMissing ? <button disabled={saving} onClick={importCurrentDocuments} className="rounded-full border border-orange-200 px-4 py-3 text-sm font-bold text-orange-700">{saving ? "Importing…" : "Import missing policies"}</button> : null}<button onClick={() => open()} className="inline-flex items-center gap-2 rounded-full bg-orange-600 px-5 py-3 text-sm font-bold text-white"><Plus className="h-4 w-4"/>New document</button></div> : null}</header>
