...
export function AdminLegalDocumentsWorkspace() {
  ...
  const importCurrentDocuments = () => void mutate(async () => {
    ...
    if (!has("customer_terms", CUSTOMER_TERMS_VERSION)) await adminLegalDocumentsClientService.create({documentKey: "customer_terms", title: "Customer Terms of Service", audience: "customer", version: CUSTOMER_TERMS_VERSION, content: plainText(customerTermsIntroduction, customerTermsSections), effectiveDate: CUSTOMER_TERMS_EFFECTIVE_DATE, lastUpdated: CUSTOMER_TERMS_LAST_UPDATED, changeSummary: "Aligns cancellation and evidence terms with the enforced Refund and Cancellation Policy", requiresAcceptance: true});
