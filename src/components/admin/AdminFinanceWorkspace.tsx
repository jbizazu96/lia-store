...
export function AdminFinanceWorkspace() {
  ...
  return <section><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-bold tracking-wide text-orange-600">FINANCE</p><h1 className="mt-1 text-3xl font-bold tracking-tight">Payouts, transfers & refunds</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">Read-only financial oversight. Transfer obligations are created after delivery; Stripe processing remains in the protected backend.</p></div><Link href="/admin/finance/lia-report" className="rounded-xl bg-orange-600 px-4 py-2.5 text-sm font-bold text-white">View LIA report</Link></div>
    ...
    <div className="mt-7 flex flex-wrap gap-2">{(["transfers", "settlements", "refunds"] as Tab[]).map((value) => <button key={value} type="button" onClick={() => setTab(value)} className={"rounded-full px-4 py-2 text-sm font-bold capitalize " + (tab === value ? "bg-orange-600 text-white" : "bg-white text-slate-600 ring-1 ring-slate-200")}>{value}</button>)}</div>
    ...
    <section className="mt-5 overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-100">{tab === "transfers" && <div className="divide-y divide-slate-100">{transfers.length === 0 ? <Empty/> : transfers.map((item) => <Row key={item.id} title={item.recipientType + " payout"} subtitle={"Order #" + (item.orderNumber || "not linked") + " · " + displayDate(item.updatedAt)} amount={cash(item.amount, item.currency)} status={item.status} note={item.lastError ? "Latest failure: " + item.lastError : item.attemptCount > 1 ? item.attemptCount + " attempts" : ""}/>)}</div>}{tab === "settlements" && <div className="divide-y divide-slate-100">{settlements.length === 0 ? <Empty/> : settlements.map((item) => <Row key={item.id} title={"Order #" + (item.orderNumber || item.id)} subtitle={"Store " + cash(item.storeAmount, item.currency) + " · Driver " + cash(item.driverAmount, item.currency) + " · " + displayDate(item.createdAt)} amount={cash(item.storeAmount + item.driverAmount, item.currency)} status={item.status}/>)}</div>}{tab === "refunds" && <div className="divide-y divide-slate-100">{refunds.length === 0 ? <Empty/> : refunds.map((item) => <Row key={item.id} title={label(item.scope) + " refund"} subtitle={"Order #" + (item.orderNumber || "not linked") + " · " + label(item.reason) + " · " + displayDate(item.updatedAt)} amount={cash(item.amount, item.currency)} status={item.status} note={item.lastError ? "Latest failure: " + item.lastError : ""}/>)}</div>}</section>
  </section>;
}
