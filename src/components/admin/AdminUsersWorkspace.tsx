...
export function AdminUsersWorkspace() {
  ...
  const create = async (event: React.FormEvent) => {
    ...
    setModalError("");
    if (draft.displayName.trim().length < 2) { setModalError("Enter the administrator's name using at least 2 characters."); return; }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(draft.email.trim())) { setModalError("Enter a valid email address."); return; }
    if (draft.password.length < 12 || !/[a-z]/.test(draft.password) || !/[A-Z]/.test(draft.password) || !/\d/.test(draft.password)) {
      setModalError("The temporary password must contain at least 12 characters, including uppercase, lowercase, and a number.");
      return;
    }
    ...
    } catch (reason) { setModalError(reason instanceof Error ? reason.message : "Unable to create the administrator."); }
  ...
  const save = async (event: React.FormEvent) => {
    ...
    setModalError("");
    if (editing.displayName.trim().length < 2) { setModalError("Enter the administrator's name using at least 2 characters."); return; }
    setWorking(editing.uid); setError(""); setMessage("");
    try {
    ...
    } catch (reason) { setModalError(reason instanceof Error ? reason.message : "Unable to update the administrator."); }
    finally { setWorking(null); }
  };
  ...
  return <section>
    <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-bold tracking-wide text-orange-600">MASTER ADMIN ONLY</p><h1 className="mt-1 text-3xl font-bold">Admin users</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">Create staff administrators and control exactly which admin areas they can open. Staff can never manage other admin users.</p></div><button type="button" onClick={() => {setDraft(emptyDraft); setModalError(""); setShowCreate(true);}} className="inline-flex items-center gap-2 rounded-full bg-orange-600 px-5 py-3 text-sm font-bold text-white"><Plus className="h-4 w-4" />Add admin user</button></div>
