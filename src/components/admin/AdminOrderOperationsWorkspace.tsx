...
const exceptionOptions = [["all", "All orders"], ["no_driver", "No driver"], ["delayed_pickup", "Delayed pickup"], ["shipday_failed", "Shipday failed"], ["cancelled", "Cancelled"]] as const;
...
export function AdminOrderOperationsWorkspace({orderId}: {orderId?: string}) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        else setOrders((await adminWorkspaceClientService.getOrders({status, exception})).orders);
  ...
  return <section>
    ...
    {error && <ErrorPanel message={error} />}{loading ? <Loading /> : <div className="mt-6 overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-100">{visibleOrders.length === 0 ? <p className="p-10 text-center text-sm text-slate-500">No paid orders match these filters.</p> : <div className="divide-y divide-slate-100">{visibleOrders.map((item) => <button type="button" key={item.id} onClick={() => router.push("/admin/orders/" + item.id)} className="flex w-full items-center gap-4 px-5 py-4 text-left hover:bg-orange-50/40"><div className="min-w-0 flex-1"><p className="font-bold text-slate-900">Order #{item.orderNumber}</p><p className="mt-1 text-sm text-slate-500">{item.storeName} · {item.customerName} · {date(item.createdAt)}</p><div className="mt-2 flex flex-wrap gap-2"><Badge value={item.status} /><Badge value={item.paymentStatus} /><Badge value={item.driverName ? item.driverName : "No assigned driver"} />{item.exceptions.map((value) => <Badge key={value} value={label(value)} warning />)}</div></div><div className="text-right"><p className="font-bold">{money(item.totalAmount, item.currency)}</p><ChevronRight className="ml-auto mt-2 h-5 w-5 text-slate-400" /></div></button>)}</div>}</div>}
  </section>;
}

function OrderDetail({detail}: {detail: AdminOrderDetail}) { return <><p className="text-sm font-bold tracking-wide text-orange-600">PAID ORDER</p><div className="mt-1 flex flex-wrap items-start justify-between gap-3"><div><h1 className="text-3xl font-bold">Order #{detail.orderNumber}</h1><p className="mt-2 text-sm text-slate-500">{date(detail.createdAt)} · {label(detail.status)}</p></div><p className="text-2xl font-bold">{money(detail.pricing.totalAmount, detail.pricing.currency)}</p></div><div className="mt-6 grid gap-4 lg:grid-cols-3"><Card icon={<UserRound />} title="Customer" values={[detail.customer.name, detail.customer.email || "", detail.customer.phone || "", detail.customer.address || ""]}/><Card icon={<PackageCheck />} title="Store" values={[detail.store.name, detail.store.phone || "", detail.store.address || ""]}/><Card icon={<Truck />} title="Delivery" values={[detail.delivery.driverName || "No LIA driver assigned", detail.delivery.shipdayStatus ? "Shipday: " + label(detail.delivery.shipdayStatus) : "Shipday not started", detail.delivery.distanceMiles ? detail.delivery.distanceMiles + " miles" : "", detail.delivery.estimatedMinutes ? "Estimated " + detail.delivery.estimatedMinutes + " min" : ""]}/></div>{detail.exceptions.length > 0 && <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><b>Needs review:</b> {detail.exceptions.map(label).join(" · ")}{detail.delivery.cancellationReason ? " — " + detail.delivery.cancellationReason : ""}</div>}<section className="mt-6 rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-100"><h2 className="font-bold">Order items</h2><div className="mt-3 divide-y divide-slate-100">{detail.items.map((item, index) => <div key={index} className="flex justify-between py-3 text-sm"><span>{item.quantity} × {item.name}</span><b>{money(item.lineTotalAmount, detail.pricing.currency)}</b></div>)}</div><div className="mt-4 grid gap-2 border-t pt-4 text-sm sm:grid-cols-2"><p>Subtotal <b className="float-right">{money(detail.pricing.subtotalAmount, detail.pricing.currency)}</b></p><p>Delivery <b className="float-right">{money(detail.pricing.deliveryFeeAmount, detail.pricing.currency)}</b></p><p>Service fee <b className="float-right">{money(detail.pricing.serviceFeeAmount, detail.pricing.currency)}</b></p><p>Tax <b className="float-right">{money(detail.pricing.taxAmount, detail.pricing.currency)}</b></p><p>Tip <b className="float-right">{money(detail.pricing.tipAmount, detail.pricing.currency)}</b></p><p className="font-bold">Total <b className="float-right">{money(detail.pricing.totalAmount, detail.pricing.currency)}</b></p></div></section><section className="mt-6 rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-100"><h2 className="font-bold">Payment & delivery timeline</h2><div className="mt-4 space-y-4">{detail.history.length === 0 ? <p className="text-sm text-slate-500">No status events recorded.</p> : detail.history.map((item, index) => <div key={index} className="border-l-2 border-orange-200 pl-4"><p className="font-bold capitalize">{label(item.status)}</p><p className="text-xs text-slate-500">{date(item.timestamp)}</p>{item.note && <p className="mt-1 text-sm text-slate-600">{item.note}</p>}</div>)}</div></section></>; }
