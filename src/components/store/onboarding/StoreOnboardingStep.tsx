...
function StoreOnboardingStepContent({ step }: { step: StoreOnboardingStep }) {
  ...
  const canSave = step === "owner"
    ? Boolean(owner.firstName.trim() && owner.lastName.trim() && owner.email.trim() && owner.phone.replace(/\D/g, "").length >= 10 && owner.address.trim() && owner.city.trim() && owner.state.trim() && owner.zip.trim() && (photo || draft.owner.photoIdUrl))
    : step === "store-information"
      ? Boolean(store.name.trim() && store.email.trim() && store.phone.replace(/\D/g, "").length >= 10 && store.description.trim() && store.address.trim() && store.city.trim() && store.state.trim() && store.zip.trim() && (logo || draft.logoUrl))
      : step === "business-information"
    ? Boolean(business.businessType && business.registeredName.trim() && business.businessStructure && (storeFront || draft.storeFrontUrl) && (storeInside || draft.storeInsideUrl))
        : step === "schedule"
  ...
  const missingRequirements =
    step === "owner"
      ? [
          ...
          !photo && !draft.owner.photoIdUrl && "photo ID",
        ]
      : step === "store-information"
        ? [
        ...
        : step === "business-information"
          ? [
              ...
              !storeFront && !draft.storeFrontUrl && "store front photo",
              !storeInside && !draft.storeInsideUrl && "inside store photo",
            ]
          : step === "schedule"
            ? [
                scheduleValidationError,
