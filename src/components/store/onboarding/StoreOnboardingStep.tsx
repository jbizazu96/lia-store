...
import { ArrowLeft, ArrowRight, CheckCircle2, Camera, Upload, AlertTriangle, RefreshCw, FileText } from "lucide-react";
...
const stepMeta: Record<StoreOnboardingStep, { title: string; subtitle: string }> = {
  ...
  agreement: { title: "Merchant agreement", subtitle: "Review and electronically acknowledge your agreement before setting up payouts." },
  stripe: { title: "Set up payouts", subtitle: "Stripe securely collects banking and identity details for payouts." },
};

const agreementAcknowledgments = [
  "I have read and understood the LIA Merchant Agreement.",
  "I have authority to enter into this Agreement for the Merchant.",
  "I agree to comply with LIA’s product-quality and food-safety standards.",
  "I understand LIA may issue refunds and recover Merchant-responsible amounts.",
  "I understand serious or repeated violations may result in suspension or termination.",
  "I agree to protect LIA customers by providing safe, fresh, accurately represented, and acceptable-quality products.",
] as const;
/*
...
function StoreOnboardingStepContent({ step }: { step: StoreOnboardingStep }) {
  ...
  const [agreementChecks, setAgreementChecks] = useState<boolean[]>(() => agreementAcknowledgments.map(() => false));
  ...
  useEffect(() => { if (!draft) return; setOwner({ firstName: draft.owner.firstName, lastName: draft.owner.lastName, email: draft.owner.email, phone: draft.owner.phone, address: draft.owner.address, city: draft.owner.city, state: draft.owner.state, zip: draft.owner.zip }); setStore({ name: draft.name, email: draft.email, phone: draft.phone, description: draft.description, address: draft.address, city: draft.city, state: draft.state, zip: draft.zip }); setBusiness({ businessType: draft.businessType, registeredName: draft.registeredName, ein: draft.ein, businessStructure: draft.businessStructure }); setSchedule(draft.schedule); setStripeStatus(draft.stripeAccountStatus); setAgreementChecks(agreementAcknowledgments.map(() => draft.merchantAgreementAcceptance?.accepted === true)); setRepresentativeName(draft.merchantAgreementAcceptance?.representativeName ?? ""); }, [draft]);
  ...
  const canSave = step === "owner"
    ...
    ? Boolean(business.businessType && business.registeredName.trim() && business.businessStructure && (!storeFrontRequired || storeFront || draft.storeFrontUrl) && (!storeInsideRequired || storeInside || draft.storeInsideUrl))
        : step === "schedule"
          ...
          : step === "agreement"
            ? agreementChecks.every(Boolean) && representativeName.trim().toLocaleLowerCase() === `${draft.owner.firstName} ${draft.owner.lastName}`.trim().toLocaleLowerCase()
          : false;
  /*
  ...
  */
      ...
      : step === "store-information"
        ...
        : step === "business-information"
          ...
          : step === "schedule"
            ...
            : step === "agreement"
              ? [
                  !agreementChecks.every(Boolean) && "all six acknowledgments",
                  representativeName.trim().toLocaleLowerCase() !== `${draft.owner.firstName} ${draft.owner.lastName}`.trim().toLocaleLowerCase() && "authorized representative's full name",
                ]
            : [];
  ...
  {step === "agreement" && <div className="space-y-5"><div className="rounded-xl border border-orange-100 bg-orange-50 p-4"><div className="flex gap-3"><FileText className="mt-0.5 h-5 w-5 shrink-0 text-orange-600" /><div><p className="font-semibold text-gray-900">LIA Merchant Agreement</p><p className="mt-1 text-sm leading-6 text-gray-600">Review the complete agreement provided by LIA before accepting. Electronic acceptance is legally significant and is recorded with your account, but you must still sign the separate paper contract with LIA.</p></div></div></div><div className="space-y-3">{agreementAcknowledgments.map((label, acknowledgmentIndex) => <label key={label} className="flex cursor-pointer gap-3 rounded-xl border border-gray-100 p-3 text-sm leading-6 text-gray-700 hover:bg-gray-50"><input type="checkbox" checked={agreementChecks[acknowledgmentIndex]} onChange={(event) => setAgreementChecks((current) => current.map((checked, indexToUpdate) => indexToUpdate === acknowledgmentIndex ? event.target.checked : checked))} className="mt-1 h-4 w-4 shrink-0 accent-orange-600" /><span>{label}</span></label>)}</div><div className="rounded-xl bg-gray-50 p-4"><Input label="Authorized representative's full legal name" required autoComplete="name" value={representativeName} onChange={(event) => setRepresentativeName(event.target.value)} placeholder={`${draft.owner.firstName} ${draft.owner.lastName}`.trim()} /><p className="mt-2 text-xs leading-5 text-gray-500">Enter your name exactly as it appears in the owner-information step. Selecting Save and continue records your electronic acceptance.</p></div></div>}
  {step === "stripe" && <div className="space-y-5"><div className="rounded-2xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-800">LIA never sees your bank account details. Stripe securely collects identity, tax, and payout information. Stripe may review your submission after you return; its status stays synchronized automatically.</div>{stripeRequired && <div className="rounded-xl bg-gray-50 p-4"><p className="font-semibold text-gray-800">Stripe status: <span className="capitalize">{(stripeStatus ?? "not_started").replaceAll("_", " ")}</span></p><p className="mt-1 text-sm text-gray-500">{returnedFromStripe || canFinishStripe ? "Your Stripe submission has been received. Submit the application only when every onboarding step is complete." : "Complete the Stripe-hosted form to continue."}</p></div>}{canFinishStripe ? <button onClick={async () => { setSaving(true); try { await storeOnboardingService.complete(user.uid); router.replace("/store/pending-approval"); } catch (cause) { setMessage(cause instanceof Error ? cause.message : "Unable to submit the store application."); } finally { setSaving(false); } }} disabled={saving} className="w-full rounded-xl bg-green-600 py-3 font-semibold text-white disabled:opacity-50"><CheckCircle2 className="mr-2 inline h-5 w-5" />{saving ? "Submitting…" : "Submit store application"}</button> : <button onClick={connectStripe} disabled={connecting} className="w-full rounded-xl bg-indigo-600 py-3 font-semibold text-white disabled:opacity-50">{connecting ? "Opening Stripe..." : stripeStatus && stripeStatus !== "not_started" ? "Continue Stripe setup" : "Connect Stripe"}</button>}</div>}
